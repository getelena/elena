...
Since both selectors now share the same baseline styles, there are no visible layout shifts, FOUC, or FOIC (Flash Of Unstyled Content, Flash Of Invisible Content). For more details, see the CSS pre-hydration styles section in the Styles guide.
...
> **Tip:** Use \`await import()\` for component modules rather than a static \`import\` statement. Elena components extend \`HTMLElement\`, which requires a Node.js shim that \`@elenajs/ssr\` installs when it loads. Dynamic imports guarantee the shim is in place first, regardless of how an import sorter may reorder your static imports.
